...
FreeImage_GetTagCount(FITAG *tag) {
	...
	return tag ? (((FITAGHEADER *)tag->data)->count == 0xFFFFFFFF ? 0 : ((FITAGHEADER *)tag->data)->count): 0;
}
...
FreeImage_GetTagLength(FITAG *tag) {
	...
	return tag ? (((FITAGHEADER *)tag->data)->length == 0xFFFFFFFF ? 0 : ((FITAGHEADER *)tag->data)->length): 0;
}
...
FreeImage_SetTagValue(FITAG *tag, const void *value) {
	if(tag && value) {
        ...
        // todo cola {
        if (tag_header->length == 0xFFFFFFFF) {
            return FALSE;
        }
        // } todo cola
